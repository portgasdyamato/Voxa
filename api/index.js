export default async function handler(req, res) {
  try {
    ...
    if (url.pathname === '/api/login') {
      const googleClientId = process.env.GOOGLE_CLIENT_ID || '33589766455-q4l78megmocn1n758mt220lek0vl617a.apps.googleusercontent.com';
      const redirectUri = process.env.GOOGLE_CALLBACK_URL || 'https://voxa-taupe.vercel.app/auth/google/callback';
    ...
    if (url.pathname === '/auth/google/callback') {
      ...
      try {
        ...
        const googleClientId = process.env.GOOGLE_CLIENT_ID || '33589766455-q4l78megmocn1n758mt220lek0vl617a.apps.googleusercontent.com';
        const googleClientSecret = process.env.GOOGLE_CLIENT_SECRET || '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';
        const redirectUri = process.env.GOOGLE_CALLBACK_URL || 'https://voxa-taupe.vercel.app/auth/google/callback';
        ...
        console.log('OAuth success for user:', userData.name);
        
        // Redirect to home page with user info
        const successUrl = `/?login=success&user=${encodeURIComponent(userData.name)}`;
        res.statusCode = 302;
        ...
        res.statusCode = 302;
        res.setHeader('Location', '/?error=oauth_callback&message=' + encodeURIComponent(error.message));
        res.end();
        return;
      }
    }

...
export function TaskCard({ task }: TaskCardProps) {
  ...
  const priorityMeta = {
    high: { color: 'text-rose-500 dark:text-rose-400', icon: AlertCircle },
    medium: { color: 'text-amber-600 dark:text-amber-400', icon: Zap },
    low: { color: 'text-emerald-600 dark:text-emerald-400', icon: Check }
  };
          ...
          <button 
            ...
            className={cn(
              ...
              task.completed 
                ...
                : "bg-background/20 border-foreground/10 hover:bg-background/40 hover:border-foreground/30"
            )}
          >
            <AnimatePresence mode="wait">
              {task.completed ? (
                ...
                <div className="w-1.5 h-1.5 rounded-full bg-foreground/20 group-hover/btn:bg-foreground/80 transition-all duration-300 group-hover/btn:scale-[1.5]" />
              )}
            </AnimatePresence>
          </button>
          ...
          <div className="flex-1 min-w-0 flex items-center justify-between gap-4">
            <div className="space-y-1.5 min-w-0 flex-1">
              <div className="flex items-center gap-3">
                <h3 className={cn(
                  "text-[14px] font-bold tracking-tight text-foreground",
                ...
                {category && (
                  ...
                  >
                    ...
                    <span className="text-[12px] font-medium text-foreground/40">{category.name}</span>
                  </div>
                )}
              </div>
              
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] sm:text-[12px] font-bold sm:font-medium text-muted-foreground/60">
            ...
            <div className="flex items-center gap-1.5 opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-200" onClick={(e) => e.stopPropagation()}>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-8 w-8 rounded-[8px] hover:bg-foreground/10 shadow-none text-muted-foreground hover:text-foreground transition-colors">
                    <MoreHorizontal className="w-4 h-4" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-40 shadow-2xl border border-border bg-popover rounded-xl p-1.5">
                  <DropdownMenuItem 
